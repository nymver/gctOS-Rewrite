...
Object.Model = "models/props/cs_office/tv_plasma.mdl"
		...
		hook.Add("PostDrawOpaqueRenderables", "gctOS_ObjPostDrawOpaqueRenderables_"..ent:EntIndex(), function() 
			if(IsValid(ent)) then
				if(ent:GetNWBool("gctOS_DedSec_Screen")) then
			        cam.Start3D2D(ent:LocalToWorld(Vector(6.15, -28, 36)), ent:LocalToWorldAngles(Angle(0,90,90)), 1)
			           surface.SetDrawColor(255, 255, 255)
			           surface.SetMaterial(Material("DedSec_Animation"))
			           surface.DrawTexturedRect(0, 0, 56, 34)
			        cam.End3D2D()
				end
			else
